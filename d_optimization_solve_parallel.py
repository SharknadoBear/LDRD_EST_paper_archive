...
# Keep each worker from spawning many hidden BLAS/OpenMP threads. This is
# especially important on Windows, where each ProcessPool worker imports NumPy
# and SciPy separately.
...
def objective_function_core(
    ...
    """Evaluate the WMCR objective for one parameter pair."""
    try:
        Ksw = 10 ** log_Ksw
        kA_mL_day = 10 ** log_kA
        ...
        wmcr_mass_interp = interp1d(
            out["t_days"],
            out["Mout_ng"],
            kind="linear",
            fill_value="extrapolate",
        )
        wmcr_conc_interp = interp1d(
            out["t_days"],
            out["Cout_ng_mL"] * 1000,  # Convert to ng/L
            kind="linear",
            fill_value="extrapolate",
        )
        ...
        return w_mass * norm_rmse_mass + w_conc * norm_rmse_conc
    except Exception:
        return 1e10


def run_optimization_attempt_worker(worker_args):
    """Run one independent Bayesian-optimization attempt in a worker process."""
    attempt_id = worker_args["attempt_id"]
    inputs = worker_args["inputs"]
    exp_mass_time = worker_args["exp_mass_time"]
    exp_mass = worker_args["exp_mass"]
    exp_conc_time = worker_args["exp_conc_time"]
    exp_conc = worker_args["exp_conc"]
    log_kA_low = worker_args["log_kA_low"]
    log_kA_high = worker_args["log_kA_high"]
    valid_log_kA_low = worker_args["valid_log_kA_low"]
    valid_log_kA_high = worker_args["valid_log_kA_high"]

    space = [
        Real(-3.0, 6.0, name="log_Ksw"),
        Real(log_kA_low, log_kA_high, name="log_kA"),
    ]

    @use_named_args(space)
    def objective_wrapper(**params):
        return objective_function_core(
            params["log_Ksw"],
            params["log_kA"],
            inputs,
            exp_mass_time,
    ...
    try:
        ...
        return {
            ...
            "error": str(exc),
            "is_valid": False,
            "wall_status": "optimization error",
        }
    ...
    out = None
    if is_valid:
        out = solve_wmcr(inputs, WMCRParams(Ksw=Ksw, kA_mL_day=kA))
...
def model_run():
    ...
    # Run independent optimization attempts in parallel until the requested number
    # of valid clusters is collected.
    cluster_results = []
    cluster_outputs = []
    attempt_count = 0
    valid_count = 0
    ...
    default_workers = max(1, min(N_cluster, available_cpus - 1 if available_cpus > 1 else 1))
    n_workers = int(os.environ.get("WMCR_N_WORKERS", default_workers))
    n_workers = max(1, min(n_workers, available_cpus))
    ...
    print(f"Parallel workers: {n_workers} (override with WMCR_N_WORKERS if needed)")

    worker_base_args = {
        "inputs": inputs,
        "exp_mass_time": exp_mass_time,
        "exp_mass": exp_mass,
        "exp_conc_time": exp_conc_time,
        "exp_conc": exp_conc,
        "log_kA_low": log_kA_low,
        "log_kA_high": log_kA_high,
    ...
    with ProcessPoolExecutor(max_workers=n_workers) as executor:
        while valid_count < N_cluster:
            remaining = N_cluster - valid_count
            batch_size = min(n_workers, max(1, remaining * 2))
            batch_args = []
            for _ in range(batch_size):
                attempt_count += 1
                worker_args = dict(worker_base_args)
                worker_args["attempt_id"] = attempt_count
                batch_args.append(worker_args)

            first_attempt = batch_args[0]["attempt_id"]
            last_attempt = batch_args[-1]["attempt_id"]
            print(
                f"\n--- Optimization attempts {first_attempt}-{last_attempt} "
                f"(valid {valid_count}/{N_cluster}) ---"
            )

            future_to_attempt = {
                executor.submit(run_optimization_attempt_worker, args): args["attempt_id"]
                for args in batch_args
            }
            pending = set(future_to_attempt)
            batch_results = []

            while pending:
                done, pending = wait(
                    pending,
                    timeout=WORKER_HEARTBEAT_SECONDS,
                    return_when=FIRST_COMPLETED,
                )
                if not done:
                    print(
                        f"  Still running {len(pending)} worker attempt(s); "
                ...
                for future in done:
                    ...
                    try:
                        ...
                        print(
                            f"Attempt {attempt_id} crashed before returning a result: {exc}",
                            flush=True,
                        )
                        raise
                    print(
                        f"  Completed {len(batch_results)}/{len(future_to_attempt)} "
                        "worker attempt(s) in this batch.",
                        flush=True,
                    )

            for attempt_result in sorted(batch_results, key=lambda item: item["attempt_id"]):
                attempt_id = attempt_result["attempt_id"]
                if attempt_result.get("failed", False):
                    failed_attempt_count += 1
                    print(f"Attempt {attempt_id} failed: {attempt_result.get('error', 'unknown error')}")
                    continue

                is_valid = attempt_result["is_valid"]
                wall_status = attempt_result["wall_status"]
                print(
                    f"Attempt {attempt_id} - Ksw: {attempt_result['Ksw']:.2e}, "
                    f"kA: {attempt_result['kA']:.2e}, "
                    f"log_kA: {attempt_result['log_kA']:.3f}, "
                    f"Objective: {attempt_result['objective']:.6f}"
                )
                will_collect_valid = is_valid and valid_count < N_cluster
                projected_valid_count = valid_count + (1 if will_collect_valid else 0)
                print(
                    f"  Valid result: {is_valid} ({wall_status}); "
                    f"valid count: {projected_valid_count}/{N_cluster}"
                )
                ...
                cluster_results.append({
                    ...
                    'attempt_id': attempt_id,
                    'Ksw': attempt_result['Ksw'],
                    'kA': attempt_result['kA'],
                    'log_Ksw': attempt_result['log_Ksw'],
                    'log_kA': attempt_result['log_kA'],
                    'objective': attempt_result['objective'],
                    'is_valid': True
                })
                cluster_outputs.append(attempt_result["output"])
    ...
    if failed_attempt_count:
        print(f"Failed attempts excluded: {failed_attempt_count}")
    ...
    output_results = {
        ...
        'optimization_settings': {
            ...
            'N_failed_attempts_excluded': failed_attempt_count,
